...
'Response.Write Request.Form("txtName")
	...
	set oRsRef = server.CreateObject("ADODB.recordset")
	strSQL = "SELECT * FROM ReferTo WHERE [Name]='" & request.Form("txtName")& "' " _
			& "AND [NIC]='" & Request.Form("txtNIC") & "';"
	'Response.Write "H1"
	oRsRef.Open strSQL, Cn
	'Response.Write oRsMax("MaxID")
	'Response.Write "H2"
	if not oRsRef.EOF then
		'Response.Write oRsRef("UserID")
		dim oRs
...
'on error resume
	...
	'-end-err
		...
		Response.Write "<br>Your User ID  is " & nUserID
		Response.Write "<br>Your Login Name is " & Request.Form("txtLoginName")
		Response.Write "<br>Your Account # is " & nNewAccID
	else
		session("Msg") = "You have no reference"
		Response.Redirect "frmNewCustomer.asp"
	end if
%>